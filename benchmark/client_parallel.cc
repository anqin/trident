...
static trident::AtomicCounter s_print_token(0);
static trident::AtomicCounter s_succeed_count(0);
static trident::AtomicCounter s_timeout_count(0);
static trident::AtomicCounter s_pending_full_count(0);
static trident::AtomicCounter s_pending_count(0);

#define MAX_PENDING_COUNT 1000

static trident::test::EchoServer_Stub* s_stub;
static trident::test::EchoRequest* s_request;
...
void StartCall()
{
    ...
    google::protobuf::Closure* done = trident::NewClosure(&EchoCallback, cntl, s_request, response);
    ++s_pending_count;
        ...
        trident::test::EchoResponse* response)
{
    ...
    if (cntl->Failed()) {
        if (cntl->ErrorCode() == trident::RPC_ERROR_SEND_BUFFER_FULL) {
            s_should_wait = true;
            ++s_pending_full_count;
        }
        else if (cntl->ErrorCode() == trident::RPC_ERROR_REQUEST_TIMEOUT) {
            s_should_wait = true;
            ++s_timeout_count;
        }
        else {
...
int main(int argc, char **argv)
{
    ...
    s_stub = new trident::test::EchoServer_Stub(&rpc_channel);

    // Create request.
    s_request = new trident::test::EchoRequest();
