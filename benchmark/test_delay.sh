...
for i in "100 0.1K" "900 1K-" "1100 1K+" "10000 10K" "100000 100K" "1000000 1M" "10000000 10M"; do
    ...
    echo
    echo "============= test $str data =============="
    echo "./$CLIENT $HOST $PORT $num $per &>$CLIENT_LOG &"
    ./$CLIENT $HOST $PORT $num $per &>$CLIENT_LOG &
    PID=$!
    SLEEP=5
    echo "sleep $SLEEP"
    sleep $SLEEP
    kill $PID
    grep -o -a 'kSofa request succeed, elapsed time in us: [0-9]*$' $CLIENT_LOG \
        | awk 'BEGIN{sum=0;num=0}{if(NR > 10){sum+=$NF;++num;}}END{ \
        print "Succeed count: " num; \
        print "Average elapsed time per request: " (sum/num) "us";}'
    sleep 1
    mv $CLIENT_LOG ${CLIENT_LOG}_${str}
done
