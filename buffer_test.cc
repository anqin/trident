...
#define TRIDENT_TRAN_BUF_BLOCK_SIZE (64u)
...
#include <trident/buffer.cc> // ATTENTION: include source file for different block size
#include <trident/tran_buf_pool.h>
#include <trident/rpc_message_header.h>
#include <trident/rpc_meta.pb.h>
#include "trident/test_data.pb.h"
...
public:
    ...
    virtual void SetUp() {
        ...
        _capacity = TranBufPool::block_size();
...
TEST_F(WriteBufferTest, Next)
{
    ...
    // no block
    {
        ...
        ASSERT_EQ(0, os->BlockCount());
        ASSERT_EQ(0, os->LastBytes());
    }
    ...
    // new block
    {
        ...
        ASSERT_EQ(_capacity, size);
        ASSERT_EQ(size, os->LastBytes());
        ASSERT_EQ(_capacity, os->TotalCapacity());
        ASSERT_EQ(_capacity, os->ByteCount());
        ASSERT_EQ(1u, os->BlockCount());
        ASSERT_EQ(_capacity, os->LastBytes());
    }
}
...
public:
    ...
    int _capacity;
    char* _block;
    trident::test::TestDataDemo _test_data;
};
...
TEST_F(PBSerDeserTest, SerDeser)
{
    ...
    trident::test::TestDataDemo new_data;
