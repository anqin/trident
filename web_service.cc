...
{
    _default_home =
        trident::NewPermanentExtClosure(this, &WebService::DefaultHome);

    RegisterServlet(ROOT_PATH, _default_home);
    RegisterServlet("/home", _default_home);

    _default_options =
        trident::NewPermanentExtClosure(this, &WebService::DefaultOptions);
    RegisterServlet("/options", _default_options);

    _default_status =
        trident::NewPermanentExtClosure(this, &WebService::DefaultStatus);
    RegisterServlet("/status", _default_status);

    _default_services =
        trident::NewPermanentExtClosure(this, &WebService::DefaultServices);
    RegisterServlet("/services", _default_services);

    _default_service =
        trident::NewPermanentExtClosure(this, &WebService::DefaultService);
...
{
    // if user register a path "/trident"
    // then "/trident/*" matched both
    std::string real_path = FormatPath(path);
                             ...
                             const HTTPRequest& request)
{
    out << "<h1>" << GetHostName(request.server_ip) << "</h1>"
        ...
        << "<b>Started:</b> " << ptime_to_string(_service_pool->RpcServer()->GetStartTime())
        << "<br>";
}
...
void WebService::ServerStatus(std::ostream& out)
{
    ...
    out << "<h3>ServerStatus</h3><hr>"
        ...
        << "<td align=\"right\">" << format_number(TRIDENT_GET_RESOURCE_COUNTER(RpcListener)) << "</td></tr>"
        << "<tr><td>countof(RpcByteStream)</td>"
        << "<td align=\"right\">" << format_number(TRIDENT_GET_RESOURCE_COUNTER(RpcByteStream)) << "</td></tr>"
        << "</table>";
}
...
void WebService::ServiceList(std::ostream& out)
{
            ...
            it != svc_list.end(); ++it)
    {
        ...
        trident::builtin::ServiceStat stat1;
        trident::builtin::ServiceStat stat60;
                            ...
                            ServiceBoard* svc_board)
{
    ...
    for (int i = 0; i < method_count; ++i)
    {
        ...
        trident::builtin::MethodStat stat1;
        trident::builtin::MethodStat stat60;
